...
**Tisp** 是静态类型、纯声明式、系统级定位的 Lisp 方言(Rust workspace,6 crate,64 源文件,26,527 行,351 测试,零编译警告)。设计思想与语言特性见 [README.md](./README.md) 与 [docs/spec.md](./docs/spec.md)。
...
| 状态 | 章节 | 说明 |
|------|------|------|
| ✅ 完全实现(30 章) | 1–10、12–18、20–32 | 全链路可用(lexer → parser → desugar → 检查 → 解释) |
| ⚠️ 部分实现(2 章) | 11 Graded Modal Types、19 Dependent Graded Types | 主要链路可用,语义深度缺(见下) |
| ⬜ 仅设计(0 章) | — | 无 |
...
- §11 Graded Modal Types:`□_r`/`◇_ε` Modal 类型接线 type_infer + 引入/消去等级推导 `resolve_modal_grade` 已就位;完整可推断情形推导(按使用次数推导 r/ε)缺
- §19 Dependent Graded Types:Pi/Sigma 变体 + 依赖等级 r+s 传播已实现;符号等级不可判定时警告放行(严格 Z3 验证缺)
...
4. **§11 完整等级推导**:补全可推断情形按使用次数推导 r/ε
5. **§19 符号等级 Z3 验证**:复用液态验证的 `verify_implication` 整数比较能力
