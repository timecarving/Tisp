...
> **效应限制**：REPL 的表达式行会先被包装为纯函数再做类型检查，因此**带 State/Signal 效应的操作（栈/状态机/数据驱动/流、`reactive-eval`）不能在提示符下直接求值**（报「效应缺失」）；请把调用写进带效应行签名的 `main` 定义、通过 `tisp --run file.tisp` 执行（见[第 13 章](13-programming-paradigms.md)）。Pure 范式（数组/符号/自动机/连接式）和 11/12 种逻辑范式可直接在提示符下使用（见[第 7 章](07-logic-programming.md)表格）。
